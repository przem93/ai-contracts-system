...
export class ContractsService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.warn("⚠️ Service will continue without embedding functionality");
    }
  }
    ...
    try {
      ...
      const result = await session.executeWrite(async (tx) => {
        ...
        for (const contractFile of contractFiles) {
          ...
          // Generate embedding for the module description if embedding service is ready
          let embedding: number[] | null = null;
          if (this.embeddingService.isReady()) {
            try {
              embedding = await this.embeddingService.generateEmbedding(contract.description);
              this.logger.log(`Generated embedding for module: ${contract.id} (${embedding.length} dimensions)`);
            } catch (error) {
              this.logger.warn(`Failed to generate embedding for module ${contract.id}: ${error.message}`);
            }
          }
